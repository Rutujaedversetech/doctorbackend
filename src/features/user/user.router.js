...
console.log(`Current date: ${year}-${month}-${day}`);
      if(role=="doctor"){
       // is_superAdmin=true
             let user=await User.create({name,email,password,age,role,is_superAdmin:true,date:date})
             console.log("user",user)
    ...
    let user=await User.create({name,email,password,age,role,is_superAdmin,date:date})
     console.log("user",user)
      return  res.status(201).send({message:"signup sucessfully"})
     } catch (error) {
