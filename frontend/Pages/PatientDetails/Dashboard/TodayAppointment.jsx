...
import { Updatefamilyinfostatus } from '../../../Redux/family/action';
...
<Td  >
          ...
          :
          (!item.visited ?item.status ?(<Button backgroundColor={'#2a9df4'} onClick={()=>handleOppointment(item._id,item,item.status)}>confirmed</Button>):
          //  :appointmentDate < currentDate ?(<Button backgroundColor={'red'}>Not visited</Button>):
          (<Button backgroundColor={'orange'} onClick={()=>handleOppointment(item._id,item,item.status)}>pending</Button>)
